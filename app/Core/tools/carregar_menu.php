...
/**************************************************************************
*
* Lê o arquivo menu.xml e atualiza a tabela tbmenu
...
class Carregar_Menu {
   ...
   */
   public function incluir_ou_alterar_menu() {
   	$arquivo = app_path().'/Core/tools/menu.xml';   	
   ...
   */
   public function existe_menu( $nome ) { 
      $resultado = false;
      $registro = DB::select(' SELECT id FROM tbmenu WHERE nome = :nome', [ 'nome' => $nome ] );
      if( $registro ) {
      	$resultado = true;
      }      
      return $resultado;
   ...
   */
   public function obter_menu( &$registro, $nome ) {       
      $registro = DB::select(' SELECT id, nome, id_pai FROM tbmenu WHERE nome = :nome', [ 'nome' => $nome ] );      
   ...
   */
   public function alterar_menu( $item ) {
      ...
      $sql  = " UPDATE tbmenu SET titulo = '{$titulo}', rota = '{$item['rota']}', acao = '{$item['acao']}' ";
   ...
   */
   public function alterar_posicao( $nome, $posicao ) {
   	//dd($nome. $posicao);   	       
      $sql  = " UPDATE tbmenu SET posicao = '{$posicao}' WHERE nome = '{$nome}' ";
      DB::update( $sql );
 	}	// alterar_posicao
   ...
   */
   public function obter_menus( &$registro ) {       
      $registro = DB::select(' SELECT id, nome FROM tbmenu' );      
   }   
